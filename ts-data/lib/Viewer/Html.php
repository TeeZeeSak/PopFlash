...
class TeamSpeak3_Viewer_Html implements TeamSpeak3_Viewer_Interface
{
  ...
  protected function getCorpusTitle()
  {
    ...
    {
      return "Uživatelů: " . $this->currObj->clientCount() . "/" . $this->currObj["virtualserver_maxclients"] . " | Online: " . TeamSpeak3_Helper_Convert::seconds($this->currObj["virtualserver_uptime"]);
    }
    elseif($this->currObj instanceof TeamSpeak3_Node_Channel && !$this->currObj->isSpacer())
    {
      return "Připoj se kliknutím";
    }
    elseif($this->currObj instanceof TeamSpeak3_Node_Client)
    {
      $client_lastconnected = $this->currObj["client_lastconnected"];
      $currenttime = time();
      $uptime = $currenttime - $client_lastconnected;
      $uptime_text = "";
      $uptimedays = floor(($uptime / 60 / 60) / 24);
      $uptimehours = floor($uptime / 3600);
      $uptimeminutes = abs(gmdate("i", $uptime % 3600));
      $uptimeseconds = abs(gmdate("s", $uptime % 3600));
      ...
      if($uptimeseconds == 1){
        $formatSeconds = "sekunda";
      }else if($uptimeseconds > 1 && $uptimeseconds < 5){
        $formatSeconds = "sekundy";
      }else{
        $formatSeconds = "sekund";
      }

      if($uptimeminutes == 1){
        $formatMinutes = "minuta";
      }else if($uptimeminutes > 1 && $uptimeminutes < 5){
        $formatMinutes = "minuty";
      }else{
        $formatMinutes = "minut";
      }
      ...
      if($uptimehours == 1){
        $formatHours = "hodina";
      }else if($uptimehours > 1 && $uptimehours < 5){
        $formatHours = "hodiny";
      }else{
        $formatHours = "hodin";
      }
      ...
      if($uptimedays == 1){
        $formatDays = "den";
      }else if($uptimedays > 1 && $uptimedays < 5){
        $formatDays = "dny";
      }else{
        $formatDays = "dnů";
      }
      ...
      if($uptimedays > 0){
        $uptimehours -= 24;
        if($uptimehours != 0){
          $uptime_text = "$uptimedays $formatDays $uptimehours $formatHours $uptimeminutes $formatMinutes a $uptimeseconds $formatSeconds";
        }else{
          $uptime_text = "$uptimedays $formatDays $uptimeminutes $formatMinutes a $uptimeseconds $formatSeconds";
        }
      }
      if($uptimehours > 0 && $uptimedays == 0){
        $uptime_text = "$uptimehours $formatHours $uptimeminutes $formatMinutes a $uptimeseconds $formatSeconds";
      }
      if($uptimehours == 0 && $uptimedays == 0){
        $uptime_text = "$uptimeminutes $formatMinutes a $uptimeseconds $formatSeconds";
      }

      return "Online: " . $uptime_text;
    }
    elseif($this->currObj instanceof TeamSpeak3_Node_Servergroup || $this->currObj instanceof TeamSpeak3_Node_Channelgroup)
    {
      ...
      $description = $this->currObj["channel_description"];
  ...
  protected function getSuffixIconChannel()
  {
    ...
    {
      $html .= $this->getImage("channel_private.png", "Zaheslován");
    }

    if($this->currObj["channel_codec"] == TeamSpeak3::CODEC_CELT_MONO || $this->currObj["channel_codec"] == TeamSpeak3::CODEC_OPUS_MUSIC)
    {
      $html .= $this->getImage("channel_flag_music.png", "Hudební kanál");
    }

    if($this->currObj["channel_needed_talk_power"])
    {
      $html .= $this->getImage("channel_flag_moderated.png", "Moderován");
    }
  ...
  protected function getSuffixIconClient()
  {
    ...
    {
      $html .= $this->getImage("client_talker.png", "Talk Power");
    }
    ...
    foreach($this->currObj->memberOf() as $group)
    {
      ...
      if(!$group->iconIsLocal("iconid") && $this->ftclient)
      {
        ...
        if($this->ftclient == "data:image")
        {
          $html .= $this->getImage("data:" . TeamSpeak3_Helper_Convert::imageMimeType($download) . ";base64," . base64_encode($download), $group, null, FALSE);
        }
        else
        {
      ...
      {
        $html .= $this->getImage("group_icon_" . $group["iconid"] . ".png", $group );
      }
    }
